...
- **CMD tests can now run inside containers, via a new "Runs On" target.** Each test gains a **Runs On** selector (Host / Containers / Host + Containers, default Host) in the test editor. **Schema v35 → v36** adds the `tests.target_type` column to databases upgrading from earlier versions (it previously existed only in the fresh-install schema, so the test list and agent command fetch errored with `no such column: target_type` on older DBs). A test targeting containers fans out across every container on each in-scope host and runs CMD checks via `docker exec` / `podman exec`; `both` also runs the host check. (Previously the `target_type` column existed but was unused.) Container metadata elements (IMAGE, PRIVILEGED, …) remain implicitly per-container. Non-CMD, non-metadata elements (FILE, PACKAGE, …) return NA inside a container rather than misreporting the host, and no-shell images (scratch/distroless) FAIL with a clear log. **Policy import/export now carries `target_type`**, so canned and shared policies preserve the Runs On target (previously dropped on round-trip).
