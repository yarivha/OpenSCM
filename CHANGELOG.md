...
- **Audit-log call sites wired up across the first batch of handlers**:
  ...
  IP-address capture is wired via a new `crate::handlers::ClientIp` extractor. Resolution order: `X-Forwarded-For` (leftmost entry — typical reverse-proxy header) → `X-Real-IP` (nginx's other forwarding header) → `ConnectInfo<SocketAddr>` (direct peer when no proxy is in front) → `"unknown"` fallback. Both `main.rs` files (CE and SaaS) switched to `into_make_service_with_connect_info::<SocketAddr>()` so the peer address actually reaches the extractor. Every audit call site now passes `Some(ip.as_str())` instead of `None`, and the IP column in the viewer becomes meaningful.

- **Daily audit-log prune** — new `audit::prune()` runs from the scheduler's main loop once per UTC day. Reads `settings.audit_log_retention_days` per tenant; tenants with retention = 0 are skipped (forever). The prune itself records one `audit.prune` row per tenant whose data was actually trimmed, with `{"removed":N,"retention_days":D}` in details, so the cleanup is itself auditable. Errors on one tenant don't block pruning for the rest. This was originally scoped as part of the broader Task #9 (retention policy across all tables) but ran in early because the setting was already exposed in Admin Settings and would otherwise have been misleading ("retention: 730 days" with no enforcement). The other tables (notifications, reports, snapshots) still wait for Task #9.

- **Audit Log cross-tenant view for Superusers** — when the caller is a Superuser (the SaaS platform-admin role), the viewer drops the per-tenant `WHERE` filter and surfaces events across every tenant. A blue `Tenant` column appears between Time and Actor, the card header shows an "All tenants" badge, and the retention figure still reflects the admin's own tenant setting. Regular Admins stay scoped to their own tenant unchanged. Needed in SaaS so platform admins can investigate customer incidents from the `default` tenant without SSH-ing into the DB.
